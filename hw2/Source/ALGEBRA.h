...
public:
	...
	MATRIX_2X2<T> Transposed() const{
		return MATRIX_2X2<T>(a11,a12,a21,a22);
	}
	...
	void SVD(MATRIX_2X2<T>& U,VECTOR_2D<T>& sigma,MATRIX_2X2<T>& V,const T tol=(T)1e-10)const{
        ...
        V.a11 = c; V.a12 = s; V.a21 = -s; V.a22 = c;
        ...
        //rearrange the U such that if there is a negative sigular value;
        //the negative singular value is the smallest one in magnitude
        sigma = VECTOR_2D<T>(M_sigma.a11, M_sigma.a22);
        
        if(sigma.x() < 0)
        { //sigma in the matrix is already sorted by magnitude
          //if sigma one is smaller than zero, need to flip to sigma 2 or entire SIGMA
            sigma = -1*sigma;
            U = -1*U;
        }
    ...
    void Delta_R(const MATRIX_2X2<T>& delta_F, MATRIX_2X2<T>& delta_R)const{
        
        MATRIX_2X2<T> U,V,S,R,Sigma(0), Other;
        VECTOR_2D<T> sig;
        SVD(U, sig, V);
        Sigma.a11 = sig.x(); Sigma.a22 = sig.y();
        S = V*Sigma*V.Transposed();
        R = U*V.Transposed();
        Other = R.Transposed()*delta_F;
        T x;
        if (fabs(S.a11 + S.a11) < 1e-17) {
            return;
        }
        x = (Other.a12 - Other.a21)/(S.a22 + S.a11);
        
        return R*MATRIX_2X2<T>(0,-x,x,0);
...
public:	
    ...
    void Delta_R(const MATRIX_3X3<T>& delta_F, MATRIX_3X3<T>& delta_R)const{
        ...
        R = U*V.Transposed();
        S = V*Sigma*V.Transposed();
        Other1 = R.Transposed()*delta_F;
        Other2(0,0) = S(1,1) + S(0,0);
        Other2(0,1) = S(1,2);
        Other2(0,2) = -S(0,2);
        Other2(1,0) = S(1,2);
        Other2(1,1) = S(0,0) + S(2,2);
        Other2(1,2) = S(0,1);
        Other2(2,0) = -S(0,2);
        Other2(2,1) = -S(0,1);
